...
export default function LandingPage() {
  ...
  const highlights = [
    {
      icon: FileText,
      title: 'Documents transformés en révision',
      description: 'Importez un PDF et obtenez une base exploitable pour le chat, les quiz et les flashcards.',
    },
    {
      icon: Zap,
      title: 'Révision active et rapide',
      description: 'Le moteur du projet ne se contente pas d’afficher le contenu: il le convertit en entraînement.',
    },
    {
      icon: Award,
      title: 'Progression visible',
      description: 'L’application suit l’XP, les streaks et les résultats pour montrer l’évolution dans le temps.',
    },
  ]
      ...
      <section className="relative pt-20 pb-24 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        ...
        <div className="mt-14 w-full max-w-6xl">
          <div className="mx-auto max-w-3xl text-center mb-8">
            <p className="text-xs uppercase tracking-[0.35em] text-indigo-300/80 font-semibold">Pourquoi le projet se démarque</p>
            <h2 className="mt-4 text-2xl sm:text-3xl font-bold tracking-tight text-white">
              Une plateforme pensée pour impressionner dès la première démonstration.
            </h2>
            <p className="mt-4 text-sm sm:text-base text-slate-400 leading-relaxed">
              SmartStudy AI combine une architecture propre, une interface visuelle marquée et un vrai parcours utilisateur.
              Le résultat donne un produit crédible, lisible et facile à présenter devant un professeur.
            </p>
          </div>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {highlights.map((item) => {
              ...
              return (
                <Card key={item.title} className="group relative overflow-hidden border border-slate-800/80 bg-slate-950/70 backdrop-blur-xl shadow-xl shadow-indigo-950/20 transition-all duration-300 hover:-translate-y-1 hover:border-indigo-500/40 hover:shadow-indigo-950/40">
                  <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 via-transparent to-cyan-400/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                  <CardContent className="relative p-6">
                    <div className="mb-5 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-cyan-500 text-white shadow-lg shadow-indigo-500/25">
